...
function generateCheckpointSuggestion(analysis) {
  const { relevantFilesChanged, linesChanged, hasTriggerPattern, changedFiles } = analysis;
...
function runValidationWorkflow() {
  try {
    ...
    if (!analysis.shouldSuggestCheckpoint) {
      console.log('ℹ️  No significant changes detected. No checkpoint suggestion needed.');
    ...
    console.log(`   Trigger pattern: ${analysis.hasTriggerPattern ? 'Yes' : 'No'}`);
    ...
    if (validationResult.success) {
      ...
      console.log(`   Command: npm run checkpoint "Auto after ${suggestion.summary}"`);
